...
def find_afterparties():
    ...
    if location and distance and measurement:
        ...
        raw_data = {query: [location, distance, measurement, sort]}
        j_data = jsonify(raw_data)
        r = requests.get("https://www.eventbriteapi.com/v3/events/search/", params=j_data)
        data = r.json()
        
        #return r.jsonify(data)
        events = []
